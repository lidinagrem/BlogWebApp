const WeatherApp = class {
    ...
    drawWeather() {
        ...
        if (this.currentWeather) {
            ...
            const dateTimeString = `${date.toLocaleDateString("en-US")} ${date.toLocaleTimeString("en-US")}`;
    ...
    createWeatherBlock(dateString, temperature, feelsLikeTemperature, iconName, description) {
        ...
        feelsLikeBlock.innerHTML = `Feels like: ${feelsLikeTemperature} °C`;
...
// Initialize the WeatherApp for current weather
document.weatherAppCurr = new WeatherApp("370e61ec5df5e6f5cac403b12665a1b4", "#weather-results-container-curr");

// Event listener for checking current weather
document.querySelector("#checkButton").addEventListener("click", function() {
    const query = document.querySelector("#locationInput").value;
    document.weatherAppCurr.getCurrentWeather(query);
});

// Initialize the WeatherApp for forecast
document.weatherAppFor = new WeatherApp("370e61ec5df5e6f5cac403b12665a1b4", "#weather-results-container");

// Event listener for checking weather forecast
document.querySelector("#checkButton").addEventListener("click", function() {
    const query = document.querySelector("#locationInput").value;
    document.weatherAppFor.getForecast(query);
});